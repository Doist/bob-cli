...
/**
 * Verifies that skills/bob-cli/SKILL.md is in sync with the generated
...
try {
    ...
    const actual = await readFile(join(root, 'skills/bob-cli/SKILL.md'), 'utf-8')

    if (actual !== expected) {
        console.error(
            'ERROR: skills/bob-cli/SKILL.md is out of sync with src/lib/skills/content.ts',
        )
        console.error('')
    ...
    console.log('skills/bob-cli/SKILL.md is in sync with content.ts')
} catch (err) {
